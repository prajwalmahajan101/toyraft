...
- [ADR-0025](0025-v1.0.0-dogfood-gate-api-additions.md) — v1.0.0 dogfood-gate API additions: `IsNotLeader` redirect predicate, `Node.NodeID()`, `Node.NotifyC()` progress signal, event-driven immediate flush on `Propose`/inbound (no longer tick-bound), and `Status()` field-semantics docs.
